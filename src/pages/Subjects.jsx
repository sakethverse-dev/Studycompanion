...
const subjectSchema = yup.object({ name: yup.string().required('Name is required'), description: yup.string(), color: yup.string() });
...
export default function Subjects() {
    ...
    // Subject modal
    const openAddSubject = () => { setEditSubject(null); setSelectedColor(COLORS[0]); sForm.reset({ name: '', description: '', color: COLORS[0] }); setShowSubjectModal(true); };
    const openEditSubject = (s) => { setEditSubject(s); setSelectedColor(s.color); sForm.reset({ name: s.name, description: s.description, color: s.color }); setShowSubjectModal(true); };
    const onSubjectSubmit = (data) => {
        const payload = { ...data, color: selectedColor };
        if (editSubject) { updateSubject(editSubject.id, payload); toast.success('Subject updated!'); }
        else { addSubject(payload); toast.success('Subject added!'); }
        setShowSubjectModal(false);
    };

    // Topic modal
    const openAddTopic = (subjectId) => { setEditTopic(null); tForm.reset({ name: '', difficulty: 'Medium', status: 'Not Started', notes: '', subjectId }); setShowTopicModal(true); };
    const openEditTopic = (t) => { setEditTopic(t); tForm.reset(t); setShowTopicModal(true); };
    const onTopicSubmit = (data) => {
        if (editTopic) { updateTopic(editTopic.id, data); toast.success('Topic updated!'); }
        else { addTopic(data); toast.success('Topic added!'); }
    ...
    return (
        <div className="page-wrapper">
            <div className="page-header">
                <h1>📚 Subjects</h1>
                <p>Organize your subjects and manage topics</p>
            </div>
            ...
            <AnimatePresence>
                {showSubjectModal && (
                    <motion.div className="modal-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowSubjectModal(false)}>
                        <motion.div className="modal" initial={{ scale: .9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: .9, opacity: 0 }} onClick={e => e.stopPropagation()}>
                            <div className="modal-header">
                                <h2>{editSubject ? 'Edit Subject' : 'New Subject'}</h2>
                                <button className="btn btn-icon btn-secondary" onClick={() => setShowSubjectModal(false)}><MdClose /></button>
                            </div>
            ...
            <AnimatePresence>
                {showTopicModal && (
                    <motion.div className="modal-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowTopicModal(false)}>
                        <motion.div className="modal" initial={{ scale: .9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: .9, opacity: 0 }} onClick={e => e.stopPropagation()}>
                            <div className="modal-header">
                                <h2>{editTopic ? 'Edit Topic' : 'New Topic'}</h2>
                                <button className="btn btn-icon btn-secondary" onClick={() => setShowTopicModal(false)}><MdClose /></button>
                            </div>
